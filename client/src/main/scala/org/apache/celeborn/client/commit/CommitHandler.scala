...
abstract class CommitHandler(
  ...
  def registerShuffle(
    ...
    //  "GetReducePartitionRequest" request of downstream reduce task here, in scenarios which support
    //  downstram task start early before the upstream task, e.g. flink hybrid shuffle.
  ...
  def collectResult(
    ...
    // todo: if support upstream task write and downstream task read simultaneously, should record the partition locations information in upstream task start time, rather than end time.
