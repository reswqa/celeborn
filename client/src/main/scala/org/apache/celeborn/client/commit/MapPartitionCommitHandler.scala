...
class MapPartitionCommitHandler(
  ...
  override def handleGetReducerFileGroup(context: RpcCallContext, shuffleId: Int): Unit = {
    // TODO: if support the downstream map task start early before the upstream reduce task, it should waiting the upstream task register shuffle, then reply these {@link GetReducerFileGroupRequest}.
