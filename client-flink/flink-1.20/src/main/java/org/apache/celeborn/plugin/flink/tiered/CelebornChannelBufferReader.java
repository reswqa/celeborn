...
public class CelebornChannelBufferReader {
  ...
  public CelebornChannelBufferReader(
    ...
    this.messageConsumer =
        requestMessage -> {
          // Note that we need to use SubPartitionReadData because the requireSegmentId is set
  ...
  public void open(int initialCredit, CompletableFuture<Boolean> openReaderFuture) {
    try {
      ...
      if (CelebornBufferStream.isEmptyStream(bufferStream)) {
        // TODO: retry open reader in scenarios that the downstream task start before than upstream
        // task, such as flink hybrid shuffle
        LOG.info("The buffer stream is empty, ignore the open process.");
        openReaderFuture.complete(false);
        return;
      }
      bufferStream.open(this::requestBuffer, initialCredit, messageConsumer, openReaderFuture);
    } catch (Exception e) {
  ...
  public void onStreamEnd(BufferStreamEnd streamEnd) {
    ...
    if (!closed && !CelebornBufferStream.isEmptyStream(bufferStream)) {
      ...
      // simultaneously
      bufferStream.moveToNextPartitionIfPossible(
          streamId, CompletableFuture.completedFuture(true), this::sendRequireSegmentId);
    }
  }
